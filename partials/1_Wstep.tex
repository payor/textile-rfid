...
\newpage
\section{Struktura prototypu anteny}
...
Celem uzyskania w trakcie symulacji parametrów zbliżonych do założeń projektu przeprowadzono parametryzację struktury. Stopień dyskretyzacji został ustalony po 6 iteracjach. Rezultaty procesu doboru wymiarów anteny, można było obserwować na wykresach. Najlepsze wyniki posłużyły do ustalenia rozmiaru projektowanej anten, a pózniej wykonania prototypu. Impedancja wejściowa symulowanej anteny ustalona została na 38 \(\Omega\). Schemat struktury po parametryzacji i prototyp widoczny jest na Rys.4.1 i Rys. 4.2. Natomiast wymiary zestawiono w Tab. 4.1. 
\noindent 
\newline Wykonana antena ma strukturę dipola o polaryzacji poziomej. 
...
Podłoże wykonano z papieru (celem bezproblemowego umieszczenia anteny do ramienia). Przenikalność elektryczną podłoża przyjęto jak dla papieru $\epsilon_{r}$ = 2.31.

\section {Struktura anteny tekstylnej}

W programie \emph{CST Microwave Studio} umieszczono prototyp anteny na zamodelowanym ramieniu. Do celów symulacji dla modelu ramienia przyjęto $\frac{2}{3}\epsilon_{r}$ i $\frac{2}{3}\sigma$ dla mięśni czyli w tym wypadku $\epsilon_{r}$ = 35,193$\frac{F}{m}$  i $\sigma$ = 1,14.
...
\section{Pomiar parametru $s_{11}$}

Pomiar parametru rozproszenia $s_{11}$
...
\begin{figure}[h!]
\centering
	\includegraphics[width=15cm]{AN_Prz_P_SYM_Prz.jpg}
...
\begin{figure}[h!]
\centering
	\includegraphics[width=15cm]{AN_Prz_P_AN_Po.jpg}
...
\begin{figure}[h!]
\centering
	\includegraphics[width=15cm]{AN_RAM_SYM_RAM.jpg}
	\caption{Wykres zależności parametru $s_{11}$ od czestotliwości dla anteny umieszczonej na ramieniu, porównany z wynikami anteny umieszczonej na ramieniu w symulacji}
\end{figure}


\chapter{Podsumowanie}
%Badania
%Zmieniająć długość L - (długość ramion promiennika) możemy regulować częstotliwość pracy anteny.
%Celem zmniejszenia impedancji skracaliśmy wartość L2.
%
